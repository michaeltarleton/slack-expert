...
def _start_callback_server() -> http.server.HTTPServer:
    """Create the callback server. Caller is responsible for serve_forever()."""
    return http.server.HTTPServer(("localhost", REDIRECT_PORT), _CallbackHandler)
...
def _save_token(token_data: dict, company: str, skill: str = "") -> tuple[Path, Path]:
    """Save token JSON and env file.

    When --skill is provided, saves to:
        ~/.claude/companies/<company>/data/tokens/slack/<skill>.json
    Otherwise (legacy), saves to:
        ~/.claude/companies/<company>/data/slack/token.json
    """
    if skill:
        data_dir = Path.home() / ".claude" / "companies" / company / "data" / "tokens" / "slack"
        data_dir.mkdir(parents=True, exist_ok=True)
        token_file = data_dir / f"{skill}.json"
    else:
...
def main() -> None:
    ...
    token_file, env_file = _save_token(token_data, args.company, skill=args.skill)
